...
async function seedDatabase() {
    try {
        ...
        // Insert accounts
        const accountsWithCustomerIds = await Promise.all(accounts.map(async account => ({
            ...account,
        ...
        const transactionsWithAccountIds = await Promise.all(transactions.map(async transaction => ({
            ...transaction,
        ...
        const userSessionsWithUserIds = await Promise.all(userSessions.map(async session => ({
            ...session,
